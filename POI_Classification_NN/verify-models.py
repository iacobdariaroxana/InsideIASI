...
model = keras.models.load_model("models/Model/model-7-epoch_10")
...
test_acc = []
for i in range(1, 11):
    model = keras.models.load_model("models/Model/model-7-epoch_{:0>2d}".format(i))
    loss, acc = model.evaluate(test_dataset)
    test_loss += [loss]
    test_acc += [acc]
    print("loss: %.2f" % loss)
...
# history loading
history = np.load("models/History/model-7-history.npy", allow_pickle=True).item()
...
plt.savefig('results/model-7-plot.png')
# plt.show()
...
confusion_matrix(test_dataset_grayscale, 7, 10)
