...
test_acc = []
for i in range(1, 16):
    model = keras.models.load_model("models/Model/model-1-end-epoch_{:0>2d}".format(i))
    loss, acc = model.evaluate(test_dataset)
    test_loss += [loss]
    test_acc += [acc]
    print("loss: %.2f" % loss)
...
# history loading
history = np.load("models/History/model-1-end-history.npy", allow_pickle=True).item()
...
plt.savefig('results/model-1(end)-plot.png')
plt.show()
...
def confusion_matrix(test_dataset, model_n, epoch):
    model = keras.models.load_model("models/Model/model-{}-end-epoch_{:0>2d}".format(model_n, epoch))
    ...
    plt.ylabel('Actual')
    plt.savefig(f"results/confusion_matrix_model-{model_n}-end_epoch-{epoch}.png")
    plt.show()


confusion_matrix(test_dataset, 1, 12)
